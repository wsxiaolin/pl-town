...
export function buildWildMushroomRestaurant(options: {
  ...
  addPart(group, new THREE.BoxGeometry(width + 0.8, base, depth + 0.2), { color: 0x75614c, roughness: 0.95, tex: 'pavement', rx: 3, ry: 2 }, [0, base / 2, 0]);
  ...
  const roofCap = addPart(group, new THREE.ConeGeometry(3.5, 1.05, 4), roof, [0, floorTop + height + 0.74, 0]);
  roofCap.scale.z = 0.65;
  ...
  const sign = addPart(group, new THREE.BoxGeometry(3.2, 0.62, 0.18), { color: 0x3f211b, roughness: 0.6 }, [0, floorTop + 2.18, frontZ + 0.04], false);
  sign.userData.restaurantPart = definition.label ?? '野生菌餐馆';
  [-1.05, -0.35, 0.35, 1.05].forEach((x) => addPart(group, new THREE.BoxGeometry(0.38, 0.08, 0.03), warm, [x, floorTop + 2.18, frontZ + 0.12], false));

  [-2.05, 2.05].forEach((x) => {
    addPart(group, new THREE.CylinderGeometry(0.09, 0.12, 0.62, 10), { color: 0xe6d0a6, roughness: 0.8 }, [x, floorTop + 0.31, frontZ - 0.2], false);
    const cap = addPart(group, new THREE.SphereGeometry(0.26, 12, 8), { color: 0xa94e32, roughness: 0.72 }, [x, floorTop + 0.72, frontZ - 0.2], false);
