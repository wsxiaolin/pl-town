...
type Glass = [string, string, string];
type CanvasDraw = (ctx: CanvasRenderingContext2D, size: number) => void;
...
export function createProceduralTextureLibrary(
  ...
  function _canvas(key: string, size: number, drawFn: CanvasDraw) {
    if (!_texCanvases[key]) {
  ...
  function _tex(key: string, rx?: number, ry?: number) {
  ...
  function addFacade(g: THREE.Object3D, texKey: string, w: number, h: number, y: number, zOffset: number, rotY?: number) {
  ...
  function _noise(ctx: CanvasRenderingContext2D, size: number, amount: number) {
  ...
  function initTextures() {
    ...
    // ══ 建筑立面贴图（完整立面，非重复）══
    function _win(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, frame: string, glass: Glass){ctx.fillStyle=frame;ctx.fillRect(x-2,y-2,w+4,h+4);const gr=ctx.createLinearGradient(x,y,x+w,y+h);gr.addColorStop(0,glass[0]);gr.addColorStop(0.5,glass[1]);gr.addColorStop(1,glass[2]);ctx.fillStyle=gr;ctx.fillRect(x,y,w,h);ctx.fillStyle='rgba(255,255,255,0.25)';ctx.fillRect(x,y,w*0.35,h*0.35);ctx.fillStyle='rgba(60,70,90,0.15)';ctx.fillRect(x+w/2-0.5,y,1,h);ctx.fillRect(x,y+h/2-0.5,w,1);}
    ...
    function _cornice(ctx: CanvasRenderingContext2D, y: number, color: string){ctx.fillStyle=color;ctx.fillRect(0,y,512,8);ctx.fillStyle='rgba(0,0,0,0.06)';ctx.fillRect(0,y+8,512,3);}
    function _awning(ctx: CanvasRenderingContext2D, y: number, a: string, b: string){const sw=512/8;for(let i=0;i<8;i++){ctx.fillStyle=i%2===0?a:b;ctx.fillRect(i*sw,y,sw,12);}ctx.fillStyle='rgba(0,0,0,0.1)';ctx.fillRect(0,y+12,512,3);}
