...
type DecorationBuilding = { group: THREE.Object3D };
type DecorationResidence = {
  id: string;
  label: string;
...
type WorldDecorationOptions = {
  scene: THREE.Scene;
  resources: ResourcePool;
  palette: { readonly SUBURB_WALL: number; readonly SUBURB_ROOF: number; readonly FIELD: number };
  roadCoords: readonly number[];
  cityLimit: number;
  buildings: readonly DecorationBuilding[];
  residences: DecorationResidence[];
...
export function createWorldDecorations(options: WorldDecorationOptions) {
  ...
  function addEdgeGrassAndPond(): void {
  ...
  function addInnerCityGreenery(): void {
  ...
  function addCityGrassPatch(cx: number, cz: number, r: number): void {
  ...
  function addDistrictBuildings(): void {
    const centers=[-33,-27,-21,-15,-9,-3,3,9,15,21,27,33], lots: [number, number, number][] = [];
    centers.forEach(x=>centers.forEach(z=>{
      if(Math.hypot(x,z)<4.8)return;
      const dist=Math.max(Math.abs(x),Math.abs(z));
      const density = dist>24 ? 0.5 : dist>12 ? 0.8 : 1;
      const offsets: readonly [number, number][] = [[0,0],[-1.35,1.15],[1.25,-1.2]];
  ...
  function addSmallBlock(x: number, y: number, z: number, type: number, i: number): void {
  ...
  function addTrees(positions: [number, number, number][]): void {
    if (!treeTrunks || !treeCrowns) {
    ...
    const trunks = treeTrunks, crowns = treeCrowns;
  ...
  function addLamps(positions: [number, number, number][]): void {
    if (!lampPosts || !lampLights) {
    ...
    const posts = lampPosts, lights = lampLights;
    positions.forEach(([x,,z]) => {
  ...
  function addBench(x: number, y: number, z: number, rotY: number): void {
  ...
  function addObelisk(x: number, y: number, z: number): void {
    const g=new THREE.Group();
  ...
  function addSignpost(x: number, y: number, z: number): void {
    const g=new THREE.Group();
  ...
  function addArch(x: number, y: number, z: number, rotY: number): void {
  ...
  function addSphereStack(x: number, y: number, z: number): void {
  ...
  function addStoneRing(x: number, y: number, z: number): void {
  ...
  function addGazebo(x: number, y: number, z: number): void {
    const g=new THREE.Group();
    const posts: readonly [number, number][] = [[0.85,0.85],[-0.85,0.85],[0.85,-0.85],[-0.85,-0.85]];
    posts.forEach(([cx,cz])=>part(g,new THREE.CylinderGeometry(0.08,0.08,1.4,10),{color:0xEDECE9,roughness:0.6,tex:'stone',rx:1,ry:1},[cx,0.7,cz]));
  ...
  function addMonolith(x: number, y: number, z: number, rotY: number): void {
    const g=new THREE.Group();
  ...
  function addSteppingStones(x: number, y: number, z: number): void {
    const steps: readonly [number, number][] = [[0,0],[0.72,0.25],[1.42,0.42],[2.1,0.25],[2.78,-0.08]];
    steps.forEach(([dx,dz])=>{
  ...
  function addHedgeRow(x: number, y: number, z: number): void {
    const g=new THREE.Group();
  ...
  function addPlanter(x: number, y: number, z: number): void {
    const g=new THREE.Group();
  ...
  function addBollards(x: number, y: number, z: number): void {
  ...
  function addStackedColumn(x: number, y: number, z: number): void {
    const g=new THREE.Group();
  ...
  function addWallSection(x: number, y: number, z: number, rotY: number): void {
    const g=new THREE.Group();
  ...
  function addBushCluster(x: number, y: number, z: number): void {
    const bushes: readonly [number, number, number][] = [[0,0,0.28],[0.5,0,0.24],[0.25,0,0.32]];
    bushes.forEach(([dx,,r])=>{
  ...
  function addPavers(): void {
    const pavers: readonly [number, number, number][] = [[-1.9,0,-1.9],[1.9,0,-1.9],[-1.9,0,1.9],[1.9,0,1.9]];
    pavers.forEach(([x,,z])=>{
  ...
  function addFlowerbed(x: number, y: number, z: number): void {
    const g = new THREE.Group();
  ...
  function addPond(cx: number, cz: number, r: number): void {
  ...
  function addSuburbHouse(x: number, z: number, rotDeg: number): void {
    const g = new THREE.Group();
    const bw = 1.2, bh = 0.9;
    // Foundation
  ...
  function addMarketStalls(x: number, z: number, count: number, dir: number): void {
