...
export function buildWushiRestaurant(options: {
  ...
  addPart(group, new THREE.BoxGeometry(width, 0.12, depth), { color: 0x8d7048, roughness: 0.82, tex: 'wood', rx: 4, ry: 3 }, [0, base + 0.06, 0]);
  const body = makeMesh(new THREE.BoxGeometry(width, height, 0.18), wall);
  body.position.set(0, base + height / 2, -depth / 2 + 0.09);
  body.castShadow = body.receiveShadow = true;
  group.add(body);
  addPart(group, new THREE.BoxGeometry(0.18, height, depth), wall, [-width / 2 + 0.09, base + height / 2, 0]);
  addPart(group, new THREE.BoxGeometry(0.18, height, depth), wall, [width / 2 - 0.09, base + height / 2, 0]);
  addPart(group, new THREE.BoxGeometry(width + 0.24, 0.18, depth + 0.24), { color: 0xa66e32, roughness: 0.64, tex: 'rooftile', rx: 4, ry: 3 }, [0, base + height + 0.09, 0]);
  ...
  [-0.78, -0.26, 0.26, 0.78].forEach((x) => addPart(group, new THREE.BoxGeometry(0.3, 0.08, 0.03), warm, [x, base + 2.43, depth / 2 + 0.18], false));

  addPart(group, new THREE.BoxGeometry(0.92, 1.72, 0.08), glass, [0, base + 0.92, depth / 2 + 0.09], false);
  ...
  [0.2, -0.08, -0.36].forEach((y, index) => addPart(group, new THREE.BoxGeometry(index === 0 ? 0.5 : 0.42, 0.055, 0.025), { color: index === 0 ? 0xb34b32 : 0x805d3c, roughness: 0.7 }, [-1.62, base + 1.02 + y, depth / 2 + 0.16], false));
  ...
  addPart(group, new THREE.CylinderGeometry(0.14, 0.14, 0.05, 20), { color: 0x3b6fe0, emissive: 0x3b6fe0, emissiveIntensity: 0.28 }, [0, base + 0.05, 0], false);
