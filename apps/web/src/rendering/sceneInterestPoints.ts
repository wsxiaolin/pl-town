...
function createCatCafeIceWall(options: SceneInterestPointOptions): SceneInterestPointEntity {
  const object = new THREE.Group();
  const ice = {
    color: 0x9eddf0,
    emissive: 0x2f7f9d,
  ...
  lowerBlocks.forEach(([x, y, z, width, height, depth], index) => {
    const block = addMesh(object, options, new THREE.BoxGeometry(width, height, depth), index % 2 ? deepIce : ice, [x, y, z]);
  ...
  upperBlocks.forEach(([x, y, z, width, height, depth], index) => {
    const block = addMesh(object, options, new THREE.BoxGeometry(width, height, depth), index === 1 ? capIce : ice, [x, y, z]);
  ...
  tag(object, 'cat-cafe-ice-wall');
  options.scene.add(object);
  return {
    id: 'cat-cafe-ice-wall',
    object,
...
export function createSceneInterestPoints(input: SceneInterestPointOptionsInput): SceneInterestPoints {
  ...
  const list = [createCatCafeNote(options), createCatCafeIceWall(options), createOrangeTree(options), createLongjingWell(options), westBeach.entity, createEchoStonePile(options), createEchoTable(options), createEchoCabin(options), createEchoDiary(options), createEchoPhotoWall(options), createEchoCabinDoor(options)];
