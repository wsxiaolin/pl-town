...
export type BuiltBuilding = BuildingDefinition & {
  group: THREE.Group;
  body?: THREE.Mesh;
...
export type ShapeBuilder = (definition: BuildingDefinition) => BuiltBuilding;
...
export function createBuildingMeshFactory(options: BuildingMeshFactoryOptions) {
  ...
  function tagMeshes(g: THREE.Group, id: string) {
    g.traverse(c => { if (c instanceof THREE.Mesh) c.userData.buildingId = id; });
  }
  function mkBodyMat(texKey: string, rx: number, ry: number) {
  ...
  function buildBank(cfg: BuildingDefinition): BuiltBuilding {
  ...
  function buildBoard(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
  ...
  function buildTower(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
  ...
  function buildDarkTower(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=1.7, bh=4.0;
  ...
  function buildPavilion(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=2.4, bh=2.3;
  ...
  function buildLibrary(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=3.0, bh=2.0;
  ...
  function buildRuins(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=2.2, bh=1.6;
  ...
  function buildSkyscraper(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=1.3, bh=6.5;
  ...
  function buildCampus(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    ...
    const campusColumns: [number, number][] = [[-0.7,0.22],[0,0.18],[0.75,0.26]];
    campusColumns.forEach(([rx,rh]) => {
  ...
  function buildKiosk(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=1.6, bh=1.5;
  ...
  function buildScreen(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=2.8, bh=3.2;
  ...
  function buildShaft(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=1.3, bh=3.8;
  ...
  function buildAltar(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw=2.0, bh=1.2;
    ...
    // Pillars at corners
    const cornerPillars: [number, number][] = [[-0.8,-0.8],[-0.8,0.8],[0.8,-0.8],[0.8,0.8]];
  ...
  function buildObservatory(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
  ...
  function buildPagoda(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    ...
    part(g, new THREE.CylinderGeometry(0.14,0.14,0.05,20), {color:P.BLUE,emissive:P.BLUE,emissiveIntensity:0.28}, [0,PLH+0.05,0], false);
    g.position.set(cfg.x,0,cfg.z); tagMeshes(g,cfg.id);
    return {...cfg, group:g, body: body!, bodyMat, labelEl:null, labelY:y+0.5};
  }
  
  // 17 MARKET — open-air stalls with striped awning
  function buildMarket(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw = 2.6, bh = 1.6;
  ...
  function buildGreenhouse(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw = 2.4, bh = 1.8;
  ...
  function buildClockTower(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw = 1.5, bh = 4.0;
  ...
  function buildTemple(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw = 2.8, bh = 1.8;
    part(g, new THREE.BoxGeometry(3.6,0.25,2.8), {color:P.BUILDING_BASE,roughness:0.8,tex:'stone',rx:2,ry:2}, [0,0.125,0]);
    const bodyMat = mkBodyMat('stone', 2, 1);
  ...
  function buildFactory(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    const bw = 3.0, bh = 1.8;
  ...
  function buildMall(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
  ...
  function buildSchool(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    // Main building
    const bw = 3.0, bh = 1.6;
  ...
  function ctx2d_windows(g: THREE.Group, x: number, y: number, z: number) {
  ...
  function buildCrown(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    // Stone base
  ...
  function buildBanana(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
  ...
  function buildQipai(cfg: BuildingDefinition): BuiltBuilding {
    const g = new THREE.Group();
    ...
    kingG.traverse(c => { if (c instanceof THREE.Mesh) c.castShadow = true; });
    g.add(kingG);
    // Queen statue
    ...
    queenG.traverse(c => { if (c instanceof THREE.Mesh) c.castShadow = true; });
    g.add(queenG);
    // Grand entrance
