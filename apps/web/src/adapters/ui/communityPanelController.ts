...
interface PhoneNotificationTemplate {
  ID: string | number;
  Subject?: { Chinese?: string };
  Content?: { Chinese?: string };
}

interface PhoneNotificationItem {
...
export function createCommunityPanelController(options: CommunityPanelControllerOptions) {
  ...
  let phoneNotificationTemplates: PhoneNotificationTemplate[] = [];

  function openPhoneApp(tab: string, kind?: SocialKind){
    setPhoneOpen(true);
    const button=document.querySelector<HTMLElement>(`[data-online-tab="${tab}"]`); button?.click();
  ...
  function handlePhysicsSessionExpired(){
    ...
    void townApiFetch('/town-api/pl/logout',{method:'POST',headers:{'x-town-pl-session':session},keepalive:true}).catch(()=>{});
  ...
  async function bindPhysicsLabAccount(event: SubmitEvent){
    ...
    const emailEl=document.getElementById('phoneBindEmail') as HTMLInputElement;const passwordEl=document.getElementById('phoneBindPassword') as HTMLInputElement;const error=document.getElementById('phoneBindError');const form=event.currentTarget as HTMLFormElement;const submit=form.querySelector<HTMLButtonElement>('button[type="submit"]');
    const email=emailEl.value.trim();const password=passwordEl.value;
    if(!email||!password){if(error)error.textContent='请输入邮箱或手机号，以及密码';return;} if(submit)submit.disabled=true;if(submit)submit.textContent='正在验证…';if(error)error.textContent='';
    try{const response=await townApiFetch('/town-api/pl/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({login:email,password})});const payload=await response.json() as { error?: string; session?: string; user?: unknown };if(!response.ok)throw new Error(payload.error||'Physics Lab 登录失败');localStorage.setItem('plSession',payload.session||'');localStorage.setItem('plUser',JSON.stringify(payload.user||{}));(document.getElementById('phoneBindForm') as HTMLFormElement).hidden=true;document.getElementById('phoneNotificationBind')?.classList.remove('session-expired');updatePhoneBindingState();showUnlockToast('Physics Lab 账号已连接');loadPhoneMessages();}catch(caught){if(error)error.textContent=caught instanceof Error?caught.message:'绑定失败';}finally{if(submit){submit.disabled=false;submit.textContent='确认连接';}}
  }
  ...
  async function loadPhoneMessages(append=false){
    ...
    let results=feed.querySelector<HTMLDivElement>('.phone-feed-results');
    ...
    try{
      const response=await townApiFetch(`/town-api/pl/notifications?skip=${phoneNotificationsSkip}&take=20`,{headers:{'x-town-pl-session':session},signal:AbortSignal.timeout(20_000)});
      ...
      const payload=await response.json() as { data?: PhoneNotificationItem[] | { $values?: PhoneNotificationItem[] }; templates?: PhoneNotificationTemplate[]; hasMore?: boolean }; const dataDir=payload.data as { $values?: PhoneNotificationItem[] } | undefined; const items=Array.isArray(payload.data)?payload.data:Array.isArray(dataDir?.$values)?dataDir.$values:[]; const templates=Array.isArray(payload.templates)?payload.templates:[];
    ...
    }catch(error){if(requestId===phoneNotificationsRequest)results.innerHTML=`<div class="phone-feed-empty">${esc(error instanceof Error?error.message:'通知暂时无法同步')}</div>`;}
  ...
  function formatNotificationText(template: string, item: PhoneNotificationItem){
  ...
  async function loadPhoneSocial(kind: SocialKind){
    ...
    try{
      const response=await townApiFetch(`/town-api/pl/social?kind=${kind}`,{headers:{'x-town-pl-session':session}}); const payload=await response.json() as { error?: string; data?: { User?: SocialUser; Statistic?: { ExperimentCount?: number; FollowerCount?: number; FollowingCount?: number }; $values?: SocialItem[] } };
    ...
    }catch(error){target.innerHTML=`<p>${esc(error instanceof Error?error.message:'社区数据暂时不可用')}</p>`;}
  }
  
  async function toggleSocialFollow(targetId: string|number, follow: boolean, button: HTMLButtonElement){
  ...
  let liveWorks: LiveWork[]=[];
  let worksLoading=false;
  let worksHasMore=false;
  let worksError='';
  let worksQuery: WorksQueryOverride | null=null;
  let worksObserver: IntersectionObserver | null=null;
  let activeWorkId='';
  let activeWorkCategory='Experiment';
  let activeWorkStarred=false;
  const worksRequests=new Map<string, Promise<{ works?: LiveWork[] }>>();
  
  function openWorkDetail(work: LiveWork | PublicWork){
    ...
    document.getElementById('workDetailStats')!.textContent=`${(work as LiveWork).visits||0} views  ·  ${(work as LiveWork).stars||0} stars  ·  ${(work as LiveWork).comments||0} comments  ·  ${(work as LiveWork).remixes||0} remixes`;
  ...
  async function loadWorkSummary(id: string){
    ...
    try{const response=await townApiFetch(`/town-api/pl/work/${id}`,{headers:{'x-town-pl-session':session,'x-town-work-category':activeWorkCategory}});if(!response.ok)return;const payload=await response.json() as { data?: WorkSummary };const data=payload.data||{};const text=data.Summary||data.Description?.[0]||data.LocalizedDescription||'';if(text&&id===activeWorkId)document.getElementById('workDetailSummary')!.textContent=text;}catch{ /* network failure: keep existing summary */ }
  }
  async function loadWorkComments(){
    ...
    try{const response=await townApiFetch(`/town-api/pl/work/${activeWorkId}/comments`,{headers:{'x-town-pl-session':session,'x-town-work-category':activeWorkCategory}});const payload=await response.json() as { error?: string; data?: { Comments?: { $values?: WorkComment[] }; $values?: WorkComment[] } };if(!response.ok)throw new Error(payload.error);const comments=payload.data?.Comments?.$values||payload.data?.$values||[];box.replaceChildren(...(comments.length?comments.map(comment=>{const row=document.createElement('article');row.className='work-comment';row.innerHTML='<b></b><p></p><small></small>';row.querySelector('b')!.textContent=comment.Author?.Nickname||'Resident';row.querySelector('p')!.textContent=comment.Content||'';row.querySelector('small')!.textContent=comment.SendDate?new Date(comment.SendDate).toLocaleDateString():'';return row;}):[Object.assign(document.createElement('p'),{className:'work-comments-empty',textContent:'No comments yet.'})]));}catch(error){box.innerHTML=`<p class="work-comments-empty">${esc(error instanceof Error?error.message:'Comments unavailable.')}</p>`;}
  }
  async function postWorkComment(event: SubmitEvent){
    event.preventDefault(); if(!activeWorkId)return; const session=localStorage.getItem('plSession'); const input=document.getElementById('workCommentInput') as HTMLInputElement; const content=input.value.trim();
    if(!session){showUnlockToast('Sign in with Physics Lab to comment.');return;} if(!content)return;
    const submit=(event.currentTarget as HTMLFormElement).querySelector<HTMLButtonElement>('button')!;submit.disabled=true;
    try{const response=await townApiFetch(`/town-api/pl/work/${activeWorkId}/comments`,{method:'POST',headers:{'content-type':'application/json','x-town-pl-session':session,'x-town-work-category':activeWorkCategory},body:JSON.stringify({content})});const payload=await response.json() as { error?: string };if(!response.ok)throw new Error(payload.error);input.value='';await loadWorkComments();showUnlockToast('Comment published.');}catch(error){showUnlockToast(error instanceof Error?error.message:'Unable to publish comment.');}finally{submit.disabled=false;}
  }
  ...
  async function loadWorkSupporters(){
    ...
    try{const response=await townApiFetch(`/town-api/pl/work/${activeWorkId}/supporters`,{headers:{'x-town-pl-session':session,'x-town-work-category':activeWorkCategory}});const payload=await response.json() as { error?: string; data?: { $values?: Supporter[] } };if(!response.ok)throw new Error(payload.error);const items=payload.data?.$values||[];box.replaceChildren(...(items.length?items.map(user=>{const row=document.createElement('article');row.className='work-comment';row.innerHTML='<b></b><p></p>';row.querySelector('b')!.textContent=user.Nickname||'Resident';row.querySelector('p')!.textContent=`Level ${user.Level||0}`;return row;}):[Object.assign(document.createElement('p'),{className:'work-comments-empty',textContent:'No supporters yet.'})]));}catch(error){box.innerHTML=`<p class="work-comments-empty">${esc(error instanceof Error?error.message:'Supporters unavailable.')}</p>`;}
  }
  async function toggleWorkSupport(){
    if(!activeWorkId)return;const session=localStorage.getItem('plSession');if(!session){showUnlockToast('Sign in with Physics Lab to support works.');return;}const button=document.getElementById('workSupport') as HTMLButtonElement;button.disabled=true;
    try{const response=await townApiFetch(`/town-api/pl/work/${activeWorkId}/support`,{method:'POST',headers:{'content-type':'application/json','x-town-pl-session':session,'x-town-work-category':activeWorkCategory},body:JSON.stringify({action:1})});const payload=await response.json() as { error?: string };if(!response.ok)throw new Error(payload.error);button.textContent='已支持';}catch(error){showUnlockToast(error instanceof Error?error.message:'Unable to support work.');}finally{button.disabled=false;}
  }
  async function toggleWorkStar(){
    ...
    const button=document.getElementById('workStar') as HTMLButtonElement;button.disabled=true;try{const next=!activeWorkStarred;const response=await townApiFetch(`/town-api/pl/work/${activeWorkId}/star`,{method:'POST',headers:{'content-type':'application/json','x-town-pl-session':session,'x-town-work-category':activeWorkCategory},body:JSON.stringify({action:next?1:0})});if(!response.ok)throw new Error('Unable to update star');activeWorkStarred=next;button.textContent=next?'★ 已点赞':'☆ 收藏/点赞';}catch(error){showUnlockToast(error instanceof Error?error.message:'Unable to update star');}finally{button.disabled=false;}
  }
  
  function openWorksPanel(context: string, queryOverride: WorksQueryOverride | null=null) {
  ...
  async function loadWorks(context: string, queryOverride: WorksQueryOverride | null=null, append=false){
    ...
    try{
      ...
      const request=queryOverride?townApiFetch('/town-api/works/query',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query})}):townApiFetch(`/town-api/works?scope=${scope}`);
