...
function snapshotBuilding(building: DamageableBuilding): BuildingSnapshot {
  ...
  if (existing && existing.meshes.every(({ mesh }) => building.group.getObjectById(mesh.id) === mesh)) return existing;
  if (existing) snapshots.delete(building.group);
...
function addRubble(group: THREE.Group, bounds: THREE.Box3): void {
  ...
  const pieces: ReadonlyArray<readonly [number, number, number, number, number, number, number]> = [
    [-size.x * 0.42, baseY + 0.14, size.z * 0.28, 0.42, 0.22, 0.3, 0.28],
    [size.x * 0.3, baseY + 0.1, -size.z * 0.35, 0.34, 0.18, 0.25, -0.42],
    [size.x * 0.05, baseY + 0.09, size.z * 0.42, 0.22, 0.14, 0.18, 0.64],
    [-size.x * 0.12, baseY + 0.07, -size.z * 0.18, 0.16, 0.12, 0.2, -0.3],
  ];
  ...
  const brokenWalls: ReadonlyArray<readonly [number, number, number, number, number]> = [
    [-size.x * 0.34, baseY + wallHeight / 2, size.z * 0.2, wallHeight, -0.08],
    [0, baseY + wallHeight * 0.38, size.z * 0.32, wallHeight * 0.76, 0.1],
    [size.x * 0.34, baseY + wallHeight * 0.62, size.z * 0.16, wallHeight * 1.24, -0.16],
  ];
  ...
  burnedInterior.position.y = baseY + 0.12;
...
function collapseBuilding(building: DamageableBuilding): void {
  ...
  const localTop = body.geometry.boundingBox?.max.y ?? height / 2;
  const originalTop = body.localToWorld(new THREE.Vector3(0, localTop, 0)).y;
  ...
  building.group.traverse((object) => {
    ...
    if (bounds.min.y > originalTop - height * body.scale.y * 0.28) mesh.visible = false;
  });
}
...
export function refreshBuildingDestroyedPresentation(building: DamageableBuilding): boolean {
  ...
  building.group.traverse((object) => {
    const mesh = object as THREE.Mesh;
    if (!mesh.isMesh || object.parent?.name === 'building-destruction-rubble') return;
    const original = snapshot?.meshes.find(item => item.mesh === mesh)?.material;
    const source = Array.isArray(original) ? original : original ? [original] : (Array.isArray(mesh.material) ? mesh.material : [mesh.material]);
    const materials = source.map(damagedMaterial);
    mesh.material = Array.isArray(mesh.material) ? materials : materials[0]!;
  });
  if (building.labelEl) {
