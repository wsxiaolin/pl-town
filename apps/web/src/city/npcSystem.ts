...
import { getNpcType, type NpcTypeLike } from './data/npcTypes';
import type { MaterialParameters } from '../rendering/meshFactory';

export interface NpcProfileLike extends NpcTypeLike {
  id: string;
  name?: string;
  head: number;
  body: number;
  home: readonly number[];
  work?: readonly number[] | null;
  workHours?: readonly number[] | null;
  hiddenHours?: readonly number[] | null;
  behavior?: string;
  spawnChance?: number;
  patrolRadius?: number;
  spawnArea?: readonly number[] | null;
  storyOnly?: boolean;
  guaranteedSpawn?: boolean;
}

export interface NpcLike {
  profile: NpcProfileLike;
  mesh: THREE.Object3D;
...
export interface NpcSystemOptions {
  scene: THREE.Scene;
  profiles: readonly NpcProfileLike[];
  npcList: NpcLike[];
  actors: {
  ...
  makeMaterial: (parameters: MaterialParameters | null) => THREE.MeshStandardMaterial;
...
export function hoursInRange(h: number, wh: readonly number[] | null | undefined): boolean {
  if (!wh) return false;
  const [s, e] = wh;
  if (s === e) return true;
...
export function isNpcHiddenAtHour(profile: NpcProfileLike, hour: number): boolean {
...
export function createNpcSystem(options: NpcSystemOptions) {
  ...
  function onYouClick() {
    ...
    if (view.cameraZoom > 6.5) { // 视角还不够大的时候才放大
      ...
      gsap.to(state, {
        z: 6.5, duration: 0.45, ease: 'power2.out', onUpdate: () => {
  ...
  function highlightPlayerMarker() {
    const cone = actors.playerMarker?.children[0] as THREE.Mesh | undefined;
    if (!cone) return;
  ...
  function addCharacters() {
    ...
    NPC_PROFILES.forEach(profile => {
      ...
      const npc: NpcLike = { profile, mesh: g, tween: null, spawnTimer: profile.spawnChance === 1 ? 0 : Math.random() * 10, idleTimer: 0 };
      npcList.push(npc);
  ...
  function randomSpawnPosition(profile: NpcProfileLike): THREE.Vector3 | null {
    if (!profile.spawnArea) return null;
    const [x = 0, z = 0, radius = 0] = profile.spawnArea;
    const angle = Math.random() * Math.PI * 2;
    const distance = Math.sqrt(Math.random()) * radius;
    return new THREE.Vector3(x + Math.cos(angle) * distance, 0, z + Math.sin(angle) * distance);
  }

  function npcDesiredTarget(npc: NpcLike) {
  ...
  function pickPatrolSpot(npc: NpcLike): THREE.Vector3 | null {
    ...
    const roadside: Array<[number, number]> = [[rx, center.z], [center.x, rz], [rx, rz]];
    roadside.forEach(([x, z]) => {
      const p = new THREE.Vector3(x, 0, z);
      if (p.distanceTo(center) <= radius && p.distanceTo(center) > 0.5) pool.push(p);
    });
  ...
  function npcYieldToPlayer(npc: NpcLike) {
    ...
    if (d < 1.05) {
      if (!npc.yielding) {
        ...
        npc.tween = gsap.to(npc.mesh.position, {
          x: dest.x, z: dest.z, duration: 0.32, ease: 'power1.out',
  ...
  function npcRoutine(npc: NpcLike) {
    ...
    npcList.forEach(npc => {
      ...
      if (behavior === 'rare') {
        ...
        if (npc.spawnTimer <= 0) {
          ...
          const appear = Math.random() < (npc.profile.spawnChance ?? 0);
  ...
  function walkAlongPath(npc: NpcLike, path: THREE.Vector3[]) {
    if (npc.walking === false) return;
    if (!path.length) {
      npc.tween = null;
      return;
    ...
    npc.tween = gsap.to(npc.mesh.position, {
      x: target.x, z: target.z, duration: dur, ease: 'power1.inOut',
  ...
  function nearestNpcTo(p: THREE.Vector3, radius: number): NpcLike | null {
    let best: NpcLike | null = null, bestD = radius;
    npcList.forEach(npc => {
      if (!npc.mesh.visible) return;
