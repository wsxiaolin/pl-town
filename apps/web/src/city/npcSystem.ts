...
export function createNpcSystem(options) {
  ...
  function createPatrolPool(center,radius) {
    ...
    for(const [x,z] of [[rx,center.z],[center.x,rz],[rx,rz]]){
      const dx=x-center.x,dz=z-center.z,distanceSquared=dx*dx+dz*dz;
      if(distanceSquared<=radiusSquared&&distanceSquared>0.25) pool.push(new THREE.Vector3(x,0,z));
    }
    return pool;
  ...
  function pickPatrolSpot(npc,gameHour) {
  ...
  function npcRoutine(npc,gameHour=getGameClock()) {
  ...
  function nearestNpcTo(p, radius) {
    let best=null, bestDistanceSquared=radius*radius;
    for(const npc of npcList){
      if(!npc.mesh.visible) continue;
