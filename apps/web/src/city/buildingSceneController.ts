...
import type { BuiltBuilding, BuildingConfig } from '../rendering/buildingMeshFactory';
import type { MaterialParameters } from '../rendering/meshFactory';

type Building = BuiltBuilding;
type BuildingDefinition = BuildingConfig & { shape: string; disabled?: boolean; facade?: string };
type ShapeBuilder = (definition: BuildingDefinition) => BuiltBuilding;
...
export function createBuildingSceneController(options: {
  ...
  addFacade: (group: THREE.Object3D, texKey: string, w: number, h: number, y: number, zOffset: number, rotY?: number) => THREE.Mesh | null;
