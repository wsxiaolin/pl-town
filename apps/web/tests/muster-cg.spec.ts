...
test('muster CG renders its continuous arc in the shared overlay and exits cleanly', async ({ page }) => {
  await waitForCityReady(page, 'cg-tester');

  const started = await page.evaluate(() => (window as any)._mini.cinematics.startMuster());
  expect(started).toBe(true);
  const overlay = page.locator('#cgOverlay');
  await expect(overlay).toHaveClass(/active/);
  // linear progress bar (continuous take, not chapter dots)
  await expect(page.locator('#cgProgress .cgm2-bar')).toBeVisible();
