  ...
  void page.addInitScript(() => {
    ...
    class GateGameWebSocket extends EventTarget {
      ...
      send(raw: string) {
        ...
        queueMicrotask(() => {
          if (stage === 'verify') {
            this.dispatchEvent(new MessageEvent('message', { data: JSON.stringify({ type: 'error', message: '该昵称已属于物实社区，请用同名账号验证。凭据只用于本次验证，不会存储。', code: 'pl-verification-required' }) }));
            return;
          }
...
test.describe('Physics Lab verification gate', () => {
  test('requires ownership verification before entering the city', async ({ page }) => {
    ...
    await expect(page.locator('#plVerifySection .pl-verify-sub')).toContainText('请用同名账号验证。凭据只用于本次验证，不会存储');
