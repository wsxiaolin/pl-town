...
for (const viewport of [{ width: 1440, height: 900 }, { width: 844, height: 390 }, { width: 390, height: 844 }]) {
  ...
  test(`pending buildings hide models, lots, labels and map entries at ${viewportLabel}`, async ({ page }, testInfo) => {
    ...
    await expect(page.locator('#memorialOverlay')).toHaveAttribute('aria-modal', 'true');
    await expect(page.locator('.ui-header')).toHaveJSProperty('inert', true);
    await expect(page.locator('#c')).toHaveJSProperty('inert', true);
    ...
    const playerBeforeMemorialInput = await page.evaluate(() => (window as any)._mini.player.position.toArray());
    await page.keyboard.down('w');
    await page.evaluate(async () => {
      for (let frame = 0; frame < 3; frame++) await new Promise<void>((resolve) => requestAnimationFrame(() => resolve()));
    });
    await page.keyboard.up('w');
    expect(await page.evaluate(() => (window as any)._mini.player.position.toArray())).toEqual(playerBeforeMemorialInput);
    await page.keyboard.press('Shift+Tab');
    await expect(page.locator('#memorialNext')).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialClose')).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialBody')).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialNext')).toBeFocused();
    ...
    await expect(page.locator('#memorialBody')).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialPrev')).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialNext')).toBeFocused();
    await page.keyboard.press('Enter');
    await page.keyboard.press('Enter');
    await expect(page.locator('#memorialPager')).toHaveText('4 / 4');
    await expect(page.locator('#memorialNext')).toBeDisabled();
    await page.keyboard.press('Tab');
    await expect(page.locator('#memorialClose')).toBeFocused();
    await page.keyboard.press('Shift+Tab');
    await expect(page.locator('#memorialPrev')).toBeFocused();
    ...
    await expect(page.locator('#memorialOverlay')).toHaveJSProperty('inert', true);
    await expect(page.locator('.ui-header')).toHaveJSProperty('inert', false);
    await expect(page.locator('#c')).toHaveJSProperty('inert', false);
