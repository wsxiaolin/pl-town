...
import { RENDER_SETTINGS, seedCityStorage, stubCityWebSocket, stubNewsstandWebSocket, waitForCityBooted, waitForCityReady } from './helpers';
...
test.describe('mobile map', () => {
  ...
  test('hides the building search box on the phone map', async ({ page }) => {
    await seedCityStorage(page, 'mobilemapsearchhidden');
    await waitForCityBooted(page);
    await expect.poll(() => page.evaluate(() => {
      const overlay = document.getElementById('mapOverlay');
      if (!overlay?.classList.contains('show')) document.getElementById('mapToggle')?.click();
    ...
    await expect(page.locator('#mapSearch')).toBeHidden();
    await expect(page.locator('#mapSearchInput')).toBeHidden();
    await expect.poll(() => page.locator('.map-icon:visible').count()).toBeGreaterThan(1);
  });
});
