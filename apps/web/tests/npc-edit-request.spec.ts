...
async function mockTownApi(page: Page): Promise<void> {
  ...
  await page.route('**/town-api/npc-edit-login', (route) => {
    ...
    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ token: body.token ? 'restored-token' : 'npc-test-token', user: { nickname: body.nickname ?? 'tester' }, registered: false }) });
  });
}
...
test('a stale stored token falls back to the sign-in form', async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem('minicityServerToken', JSON.stringify({ token: 'stale-token', user: { nickname: 'old' } })));
  await page.route('**/town-api/npc-edit-catalog', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(CATALOG) }));
