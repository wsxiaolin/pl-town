...
try {
  ...
  send(alice, { type: 'progress.reward.claim', rewardId: 'ice_accept', claimSequence: 1 });
  const lemonadeReward = await waitFor(alice, 'progress.updated', (message) => message.event?.rewardId === 'ice_accept' && message.event.claimed === true);
  if (lemonadeReward.progress.inventory.ice_lemonade !== 1) throw new Error('Ice accept reward must grant one iced lemonade');
  send(alice, { type: 'progress.reward.claim', rewardId: 'ice_accept', claimSequence: 1 });
  const retriedLemonadeReward = await waitFor(alice, 'progress.updated', (message) => message.event?.rewardId === 'ice_accept' && message.event.claimSequence === 1 && message.event.claimed === false);
  if (retriedLemonadeReward.event.accepted !== true || retriedLemonadeReward.progress.inventory.ice_lemonade !== 1) throw new Error('Retrying an acknowledged Ice reward must not grant a duplicate item');
  send(alice, { type: 'progress.item.consume', itemId: 'ice_lemonade', quantity: 1 });
  const consumedLemonade = await waitFor(alice, 'progress.updated', (message) => message.event?.type === 'item.consumed' && message.event.itemId === 'ice_lemonade');
  if ('ice_lemonade' in consumedLemonade.progress.inventory) throw new Error('The cat cafe ice wall must be able to consume the iced lemonade reward');
  send(alice, { type: 'progress.reward.claim', rewardId: 'ice_accept', claimSequence: 2 });
  const repeatedLemonadeReward = await waitFor(alice, 'progress.updated', (message) => message.event?.rewardId === 'ice_accept' && message.event.claimed === true);
  if (repeatedLemonadeReward.progress.inventory.ice_lemonade !== 1) throw new Error('Every Ice accept ending must grant another iced lemonade');
  send(alice, { type: 'progress.reward.claim', rewardId: 'ice_reject', claimSequence: 1 });
  const crownReward = await waitFor(alice, 'progress.updated', (message) => message.event?.rewardId === 'ice_reject' && message.event.claimed === true);
  send(alice, { type: 'progress.reward.claim', rewardId: 'ice_reject', claimSequence: 2 });
  const repeatedCrownReward = await waitFor(alice, 'progress.updated', (message) => message.event?.rewardId === 'ice_reject' && message.event.claimed === true && message.progress.inventory.ice_wet_crown === 2);
  if (crownReward.progress.inventory.ice_wet_crown !== 1 || repeatedCrownReward.progress.inventory.ice_wet_crown !== 2) throw new Error('Every Ice reject ending must grant another wet crown');
  send(alice, { type: 'progress.achievement.unlock', achievementId: 'cat_death_remembrance' });
  const catDeathAchievement = await waitFor(alice, 'progress.updated', (message) => message.event?.type === 'achievement.unlocked' && message.event.achievementId === 'cat_death_remembrance');
  if (!catDeathAchievement.progress.achievements.includes('cat_death_remembrance') || catDeathAchievement.progress.currency !== 1160) throw new Error('Completing Cat Death must persist its zero-currency remembrance achievement');
