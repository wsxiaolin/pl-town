...
export function voteCity(user: User, body: Record<string, unknown>) {
  ...
  return db.transaction(() => {
    ...
    if (!existing) {
      const row = db.prepare('SELECT built FROM city_projects WHERE id = ?').get(projectId) as { built: number };
      ...
      revision = (db.prepare('SELECT revision FROM city_meta WHERE id = 1').get() as { revision: number }).revision;
    ...
    return result(revision!, Boolean(existing));
  }).immediate();
}
