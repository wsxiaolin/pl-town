...
- 物实账号登录（可选）：`PHYSICS_LAB_OAUTH_AUTHORIZE_URL`、`PHYSICS_LAB_OAUTH_PUBLIC_ORIGIN`（两者同时配置才启用）、`PHYSICS_LAB_OAUTH_CLIENT_ID`/`PHYSICS_LAB_OAUTH_CLIENT_SECRET`（默认复用物实公共 `community` 客户端）、`PHYSICS_LAB_OAUTH_STATE_TTL_MS`。
