...
- 物实账号登录（可选）：`PHYSICS_LAB_OAUTH_AUTHORIZE_URL`、`PHYSICS_LAB_OAUTH_PUBLIC_ORIGIN`、`PHYSICS_LAB_OAUTH_CLIENT_ID`、`PHYSICS_LAB_OAUTH_CLIENT_SECRET`（四项同时配置才启用）、`PHYSICS_LAB_OAUTH_STATE_TTL_MS`。client id 与 client secret 必须通过部署环境变量注入。
