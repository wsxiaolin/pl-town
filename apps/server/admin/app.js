...
const truncate = (value, limit = 32) => { const text = String(value); return text.length > limit ? `${text.slice(0, limit)}…` : text; };
...
function showApp(session) {
  ...
  $('#actorName').textContent = truncate(state.actor, 48); $('#loginView').hidden = true; $('#appView').hidden = false;
...
async function loadOffsiteBackups() {
  ...
  const rows = data.items.map((backup) => {
    ...
    restore.addEventListener('click', () => confirmAction('恢复异地备份', `将从阿里云 OSS 下载 ${backup.name} 并覆盖当前数据库，所有在线居民会被强制下线并需重新登录。确定继续？`, () => restoreOffsiteBackup(backup.name)));
    ...
    group.append(restore, link, del); actions.append(group);
