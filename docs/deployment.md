...
# 可选：物实账号 OAuth2 登录。以下四项同时填写才启用；client id 和
# client secret 只能从部署环境变量或密钥管理器注入。
...
物实账号登录是可选功能：同时配置四项 `PHYSICS_LAB_OAUTH_*` 环境变量后，登录弹层会出现「用物实账号登录」。请确认公开 Origin 已在物实服务的回调白名单内。OAuth 首次登录会以物实昵称创建无密码居民，若昵称已被小城密码账号占用则拒绝登录。
...
- 执行 WAL 检查点。
- 创建、下载和重新校验数据库备份。
- 经二次确认后从本地备份或异地 OSS 备份在线恢复；恢复期间拒绝并发恢复和自动备份。

后台故意不提供任意 SQL。控制台在线恢复会覆盖当前库并强制下线所有居民。灾难恢复或需要整库文件替换时，应先停服并在终端执行 CLI。
...
控制台「备份」页可对本地或异地 OSS 备份做在线恢复：服务保持运行，会强制下线所有居民、覆盖当前库并撤销会话。异地备份本机缺失时会临时拉取到本地校验，临时文件不会进入本地备份列表。并发恢复与恢复期间的自动/手动备份会被拒绝。

灾难恢复或需要整库文件替换时，使用下面的停服 CLI。
